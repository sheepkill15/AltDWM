...
- **Tiling WM** (`src/manager.rs:84`): `EnumWindows` + live visibility, `GetWindowPlacement`, minimize/hide transitions, `DwmGetWindowAttribute(DWMWA_CLOAKED)`, `WS_EX_TOOLWINDOW`, and `GA_ROOT`. Hidden or minimized startup windows are retained in stable task-list order without consuming a tile. Per-monitor `MonitorFromWindow`/`GetMonitorInfoW`; atomic `BeginDeferWindowPos`/`DeferWindowPos`.
...
- **Real notification area** (`src/tray.rs`): AltDWM registers the `Shell_TrayWnd` class and receives `Shell_NotifyIcon` traffic itself, so the tray shows applications' own `HICON`s and tooltips rather than a mirror of Explorer's buttons. Add/modify/delete/setversion are honoured by `(hWnd, uID)` or `guidItem`; clicks are posted back as the callback the application registered, in the `NOTIFYICON_VERSION_4` shape where it asked for one; left, right, and double click all reach it. The installed elevated tray host permits the narrowly scoped `WM_COPYDATA` transport so normal-integrity applications can register their icons. `TaskbarCreated` is broadcast once the native taskbar is out of the way and again on exit, so icons come to AltDWM at startup and return to Explorer on shutdown. `NIS_HIDDEN` icons and any that do not fit go behind an overflow button. `general.tray` selects `auto` (the default), `native`, `explorer`, or `off`; `--list-tray` hosts the tray for three seconds and prints what arrives.
- **Shell startup processing and app activation** (`src/startup.rs`, `src/apps.rs`): when AltDWM is the configured Winlogon shell, it launches enabled per-user and machine `Run` entries plus user/common Startup-folder items after the notification-area host exists. The installed elevated shell uses a per-session, normal-integrity scheduled helper for both startup processing and command-center launches. Classic shell entries use `ShellExecuteW`; packaged AUMIDs such as ChatGPT and Terminal use `IApplicationActivationManager`, avoiding intermittent AppsFolder error 5. Administrator requests originate from the normal-integrity helper so UAC verbs work; packages without an elevated verb fall back to normal activation. Windows' `StartupApproved` disabled state is respected, and a volatile per-logon marker prevents duplicate launches if AltDWM restarts. Disable startup processing with `general.launch_startup_apps=false`; inspect the resolved set with `--list-startup`.
...
- **Hotkeys** (`src/main.rs`): Dynamic `RegisterHotKey` from `[[keybinds]]` (default `Alt+Shift+Space` command center; `R` retile, `T` toggle, `Q` quit, `G/M/F/S` layouts, `C` reload, `J/K` focus, `Y` floating, `N/P` move monitor). Configured `Win+…` chords use a low-level keyboard hook and are consumed before Windows or the focused app can receive the primary key.
