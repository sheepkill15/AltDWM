...
- **Widgets** (`src/widgets.rs` trait): `clock`, live `layout` status, real `workspaces` pills, `window_list` (including minimized windows; click to focus/minimize/restore), `window_title`, a real `tray` hosting `Shell_NotifyIcon` (three visible items by default, configurable overflow, no duplicate system-volume control), a configurable combined `system_status` cluster, the compatible individual `volume`/`battery`/`network`/`input` widgets, `spacer`, `launcher`, and Rhai `custom` status widgets.
