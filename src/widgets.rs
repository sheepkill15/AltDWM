...
fn window_pill_width(title: &str) -> i32 {
    (title.chars().count() as i32 * 7 + 30).min(140)
}
...
impl Widget for WorkspacesWidget {
    ...
    fn draw(&self, hdc: HDC, rect: RECT, _ctx: &PanelCtx) {
        unsafe {
            ...
            let layout = crate::CURRENT_LAYOUT
                ...
                .unwrap_or_else(|e| e.into_inner())
                .name();
            let tiling = if crate::TILING_ENABLED.load(std::sync::atomic::Ordering::SeqCst) {
                ""
            } else {
                " [PAUSED]"
            };
            // include panel monitor context
            let txt = format!("TILED {} | {}{} ", count, layout, tiling);
    ...
    fn on_click(&self, x: i32, _y: i32, _ctx: &PanelCtx) -> Option<String> {
        let _ = x;
        // The documented IVirtualDesktopManager API can inspect, but cannot
        // switch desktops. Keep this status widget non-interactive until the
        // optional internal virtual-desktop integration exists.
        None
    }
}
...
impl Widget for WindowListWidget {
    ...
    fn on_click(&self, x: i32, _y: i32, ctx: &PanelCtx) -> Option<String> {
        let wins = {
            let tb = crate::taskbar::get_taskbar_hwnd();
            let mut v = crate::manager::collect_windows(tb);
            v.retain(|w| !crate::rules::is_floating(*w) && !crate::focus::is_runtime_floating(*w));
            v.retain(|w| crate::virtual_desktop::is_on_current_desktop(*w));
            v
        };
        if wins.is_empty() {
    ...
    fn on_click(&self, _x: i32, _y: i32, _ctx: &PanelCtx) -> Option<String> {
        Some(
            self.cfg
                ...
                .unwrap_or_else(|| "launch('explorer.exe')".into()),
        )
    }
}
...
mod tests {
    ...
    fn pill_width_is_character_based_and_capped() {
        assert_eq!(window_pill_width("абв"), 51);
        assert_eq!(window_pill_width(&"x".repeat(100)), 140);
    }
}
